package se.etimo.slack.reimplemented
...
object FileHandler {
  ...
  def slackFileInfoGen(file: com.slack.api.model.File): SlackFileInfo = {
    val slackInfo = SlackFileInfo(
      fileId = file.getId(),
      name = file.getName(),
      size = file.getSize().toLong,
      urlPrivate = file.getUrlPrivate,
      isPublic = file.isPublic(),
      mimeType = Option(file.getMimetype()),
      thumb_info = findThumbUrl(file, maxSize = 800),
      preview = Option(file.getPreview()),
      previewHighlight = Option(file.getPreviewHighlight()),
      permalink = Option(file.getPermalink()),
      permaLinkPublic = Option(file.getPermalinkPublic()),
      title = Option(file.getTitle())
    )
    slackInfo
  }
  ...
  private def findThumbUrlJs(js: JsValue, maxSize: Int = 800): Option[(Int, String)] = {
  ...
  private def findThumbUrl(file: com.slack.api.model.File, maxSize: Int = 800): Option[(Int, String)] = {
    Option(800, file.getThumb800Gif())
  }
